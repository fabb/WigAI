...
(Details are in `../architecture.md` and `component-view.md`)
...
* **File Structure:** Adhere to the layout defined in `project-structure.md`.
* **Java Features:**
...
* **MCP Responses for Errors:**
    * If an MCP command cannot be processed successfully (e.g., invalid parameters, Bitwig action fails), WigAI should attempt to return a structured MCP error response to the client, as defined in `api-reference.md`. The MCP Java SDK might provide utilities for this.
...
* **Process:**
    ...
    2.  **Execution:**
        * Use a simple MCP client (e.g., a Python script, Node.js script, `curl`, or a tool like Postman) to send MCP tool calls (as defined in `api-reference.md`) to the WigAI server endpoint.
