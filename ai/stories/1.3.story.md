...
- [ ] Set up MCP Java SDK
  ...
  - [ ] Set up the SDK's server with SSE transport (Streamable HTTP not yet supported)
  - [ ] Configure the SDK's JSON-RPC 2.0 message processing
  - [ ] Set up proper error handling using the SDK's facilities

- [ ] Implement standard MCP ping functionality
  - [ ] Add the standard ping handler to the MCP server
  - [ ] Implement the response generation according to the specification
  - [ ] Add logging for ping requests and responses

- [ ] Implement the "status" tool
  - [ ] Create `StatusTool` class implementing the SDK's tool interface
  - [ ] Define appropriate input/output schema using SDK utilities
  - [ ] Implement the tool handler to return the version-based status response
...
- [ ] Implement version retrieval
  ...
  - [ ] Use the version in the "status" response

- [ ] Test SDK-based MCP endpoints
  - [ ] Verify the standard ping functionality
  - [ ] Verify the SDK's built-in `tools/list` endpoint works correctly
  - [ ] Verify the SDK's built-in `tools/call` endpoint with the "status" tool
